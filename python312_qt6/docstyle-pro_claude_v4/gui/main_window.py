...
class HelpDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("DocStyle Pro 사용 방법")
        ...
        self.setStyleSheet("""
            ...
            h2 { color: #1E293B; border-bottom: 2px solid #CBD5E1; padding-bottom: 8px; margin-bottom: 15px; font-size: 20px;}
            h3 { color: #2563EB; margin-top: 25px; margin-bottom: 5px; font-size: 16px; font-weight: bold;}
            ul { margin-top: 8px; margin-bottom: 20px; padding-left: 20px; }
            li { margin-bottom: 8px; color: #334155; }
            code { background-color: #E2E8F0; padding: 2px 6px; border-radius: 4px; font-family: monospace; font-size: 13px; color: #B91C1C; }
            QPushButton {
        ...
        help_text = QLabel("""
            ...
            <h3>1. 파일 로드 및 작성</h3>
            <ul>
                <li><b>파일 로드 탭:</b> 작성된 <code>.md</code>(마크다운), <code>.docx</code>(워드) 파일을 점선 영역에 드래그 앤 드롭하세요.</li>
                <li><b>직접 작성 탭:</b> 붙여넣기나 직접 타건하여 글을 작성하세요. 노션(Notion)처럼 마크다운 문법(# 제목, - 리스트, > 인용)을 지원합니다.</li>
                <li>오른쪽 '실시간 미리보기' 창에서 서식이 어떻게 적용되는지 바로 확인할 수 있습니다.</li>
            </ul>

            <h3>2. AI 원고 정리기 (선택 사항)</h3>
            <ul>
                <li><b>설정:</b> [⚙️ 설정] 버튼을 눌러 OpenAI, Claude, Gemini, Groq 등의 API 키를 입력하세요.</li>
                <li>
                    <b>💡 Groq API 키 발급 방법 (초보자용):</b><br>
                    ① 브라우저에서 <a href="https://console.groq.com/" style="color:#2563EB;">console.groq.com</a> 에 접속합니다.<br>
                    ② 구글(Google) 계정 등으로 간편 로그인을 진행합니다.<br>
                    ③ 왼쪽 메뉴에서 <b>[API Keys]</b> 탭을 클릭합니다.<br>
                    ④ <b>[Create API Key]</b> 버튼을 누르고 이름(예: DocStyle)을 입력해 제출합니다.<br>
                    ⑤ 화면에 나타나는 <code>gsk_...</code> 로 시작하는 긴 텍스트를 복사하여 이 앱의 [⚙️ 설정] -> Groq API Key 칸에 붙여넣으세요.
                </li>
                <li><b>AI 초안/목차 생성:</b> 좌측 하단의 설정 패널에서 '책 제목'을 입력한 뒤, '📝 직접 작성' 탭의 [AI 목차 기획] 또는 [AI 초안 자동 생성] 버튼을 누르면 인공지능이 글을 자동으로 써줍니다.</li>
                <li><b>문장 다듬기:</b> 에디터의 텍스트를 <b>드래그 후 우클릭</b>하면 문장을 다듬거나 요약할 수 있는 AI 메뉴가 뜹니다.</li>
            </ul>

            <h3>3. 내 소스 보관함 (Vault)</h3>
            <ul>
                <li>가장 왼쪽에 위치한 📚 <b>내 소스 보관함</b> 패널은 레퍼런스 자료를 모아두는 곳입니다.</li>
                <li>로컬의 <code>Documents/DocStyle_Vault</code> 폴더에 자동 저장되며, AI가 이 자료들을 참고하여 글을 쓰거나 질문에 답할 수 있습니다.</li>
                <li>가운데 <b>💡 인사이트 랩</b> 탭을 클릭해 보관함의 문서를 바탕으로 AI와 대화해 보세요.</li>
            </ul>

            <h3>4. 템플릿 선택 & 워드 변환</h3>
            <ul>
                <li>가운데 <b>📚 템플릿 라이브러리</b> 탭에서 원하는 출판/테마 템플릿을 선택하세요. 각 템플릿마다 고유한 색상과 폰트가 적용됩니다.</li>
                <li>좌측 하단의 <b>'③ 변환 실행'</b> 영역에서 [🚀 변환 시작]을 클릭하면, 선택된 템플릿 서식이 적용된 시각적으로 완벽한 워드(.docx) 파일이 생성됩니다.</li>
            </ul>
        """)
